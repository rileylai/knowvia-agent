...
| 2.0 | Generic Knowledge Contract | manual_verification | 定義所有 source 共用的最小 ingestion、chunk、embedding、retrieval 與 citation contract。 | Generic `SourceDocument`、`KnowledgeChunk`、retrieval eligibility 與 backend citation contract，支援 `notion`、`pdf`。 | Automated: Contract、eligibility、retrieval、citation 與 Notion regression passes.<br>Manual: Combined with 2.1 source flow. | Automated implementation、local indexed PDF retrieval 與 bounded QA 已完成；等待本輪 frontend 人工驗收。 |
| 2.1 | PDF Knowledge Pipeline | manual_verification | 先完成一條可驗收的 generic source path。 | PDF → `SourceDocument` → `KnowledgeChunk` → embedding → retrieval → citation → frontend verification。 | Automated: PDF indexing, retrieval and citation regression passes.<br>Manual: Upload PDF → indexing → Chat question → citation. | 三份 sample PDF 已完成正式 indexing；bounded positive/insufficient-info QA 已通過，等待使用者人工驗收。 |
| 2.1.1 | PDF Positive QA Retrieval/Sufficiency Fix | manual_verification | 確保 PDF enumeration question 有足夠 generic retrieval context，且不放寬 `insufficient_info` guardrail。 | QA default retrieval coverage 調整為 `top_k=10`；保留 PDF eligibility、backend citation 與 zero-citation invariant。 | Automated: PDF QA default top-k positive contract、irrelevant evidence insufficient-info regression passes.<br>Manual: A/B/C PDF probes and Chat positive questions. | Bounded A/B/C diagnostic 判定為 retrieval coverage + evidence sufficiency combination；未修改 threshold、prompt 或 retrieval algorithm。 |
| 2.1.2 | PDF Index Result Chunk Count | manual_verification | 讓 Knowledge success card 顯示 backend 實際 indexed chunk count。 | Success state 顯示 filename、source kind、status、indexed/embedded chunk counts。 | Automated: Successful PDF indexing renders backend chunk count.<br>Manual: Upload PDF → verify deterministic count. | UI implementation 與 frontend test 已存在；等待使用者 browser confirmation。 |
